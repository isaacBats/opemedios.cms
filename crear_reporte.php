...
//
// Funcion que regresa el tr correspondiente mediante variables estaticas
//
//
...
$pdf->SetSubject('Reporte de Noticias');
$pdf->SetKeywords('TCPDF, PDF, Reporte, Opemedios, clientes');

// set default header data
$pdf->SetHeaderData('logo_pdf.jpg', 40, 'OPEMEDIOS', 'Noticias del Cliente: '.$nempresa['name'].'.');
...
$htmlcode = '<p>
<span style="font-weight: bold; font-size:large;">III. Noticias:</span><br><br>
...
// Detalle Noticias
$htmlcode = '<p><span style="font-size:medium; text-decoration:underline;">3.2 Detalle Noticias:</span><br>';
        foreach($array_temas_nombre as $value => $label){
            $htmlcode.='
			...
			<td>
				<table border="1" cellspacing="0" cellpadding="0">
					<tr align="center" style="background-color:#eaf5f6; font-weight:bold;">
						...
						<td width="35">Id</td>
						<td width="120">Encabezado</td>												
						<td width="150">Síntesis</td>
						<td width="42">Tendencia</td>
            ...
            while($row = $base->get_row_assoc()){
                $htmlcode.= '<tr align="center" style="font-size:small;">
								...
								<td width="50">'.$row['fuente'].'</td>
								<td width="35">'.$row['id_noticia'].'</td>
								<td width="120">'.$row['encabezado'].'</td>
                                <td width="150">'.WordLimiter($row['sintesis'],100).'</td>                                
								<td width="42">'.$row['nombre_tendencia'].'</td>';
				if ($row['tipo'] == 1) {
				...
				$htmlcode.= '<td width="50">'.$row['fecha'].'</td>
                            </tr>';
            }
