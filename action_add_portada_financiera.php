...
$url = "data/financiero"; // donde se copian los documentos en el servidor
...
else
{
    ...
    {
        $mensaje = "exito";
        $thumbnail = new thumbnail($path,$urlThunmb,120,180,90,"_pp.");
...
$base->close();
header("Location: agrega_portadafinanciera.php?&error_upload=".$error_upload."&mensaje=".$mensaje);
}
/****************************************************************************
 *
...
echo $query = "SELECT * FROM portada_financiera WHERE id_portada_financiera =".$row_id." LIMIT 1;";

$base->execute_query($query);
$pPlana = new Financiero($base->get_row_assoc());
...
$base->close();
header("Location: edit_financiero.php?id_pp=".$row_id."&error_upload=".$error_upload."&mensaje=".$mensaje);
}
else
{
   $base->close();
