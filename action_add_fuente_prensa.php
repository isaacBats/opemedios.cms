<?php
/*
 * Action para agregar una nueva fuente de periodico al sistema. recoje datos del formulario y
 * llama a la stored function  NEW_FUENTE_EXTRA despues manda a la pagina ver_fuente_periodico.php donde se puede continuar
...
$nueva_fuente = new FuenteExtra($datos_fuente,3);
...
if($base->init()!=0)
{
    ...
    if($base->execute_query($nueva_fuente->SQL_NEW_FUENTE_EXTRA())!=0)
    {
        ...
        header("Location: ver_fuente_periodico.php?id_fuente=".$nueva_fuente->get_id());
        exit ();
    }
    else
    {
