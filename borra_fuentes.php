...
$base->execute_query("SELECT * FROM fuente WHERE id_Fuente = '".$id_fuente."'");
$numero_rows = $base->num_rows();

if ($numero_rows > 0) {
	$row_fuente = $base->get_row_assoc();
	$nombre_fuente = $row_fuente["nombre"];
	$tipo_fuente = $row_fuente["id_tipo_fuente"];
	$base->execute_query("DELETE FROM fuente WHERE id_Fuente = '".$id_fuente."'");
	//$rows_afectados = $base->affected_rows();
	if ($tipo_fuente == 1)
		$base->execute_query("DELETE FROM fuente_tel WHERE id_Fuente = '".$id_fuente."'");
	if ($tipo_fuente == 2)
		$base->execute_query("DELETE FROM fuente_rad WHERE id_Fuente = '".$id_fuente."'");
	if ($tipo_fuente == 3)
		$base->execute_query("DELETE FROM fuente_per WHERE id_Fuente = '".$id_fuente."'");
	if ($tipo_fuente == 4)
		$base->execute_query("DELETE FROM fuente_rev WHERE id_Fuente = '".$id_fuente."'");
	if ($tipo_fuente == 5)
		$base->execute_query("DELETE FROM fuente_int WHERE id_Fuente = '".$id_fuente."'");		
}

// mandamos a llamar el codigo donde se obtienen los clientes  ya con paginacion
include("phpdelegates/paginacion_fuentes.php");
...
<table width="1000" border="0">
                        ...
                        <tr>
                            <td valign="top">
								<form id="form_buscar" name="form_buscar" method="get" action="admin_fuentes.php">
									<table width="152" border="0" align="center" cellpadding="0" cellspacing="0">
                                        <tr>
                                            <td height="28" background="images/images/menusec_01_busqueda.png">&nbsp;</td>
                                        </tr>
                                        <tr>
                                            <td height="55" align="center" valign="middle" background="images/images/menusec_02_empty.png">
											<label>
							...
							<td valign="top">                               
                                <table width="800" border="3" align="center" cellpadding="0" cellspacing="0">
                                    <tr class="header2">
                                        <td width="32">Borrado de Fuentes</td>
									</tr>
									<tr>
                                        <td width="32">&nbsp;</td>
									</tr>
									<tr>
                                        <td width="32">La Fuente <b><?php echo $nombre_fuente; ?></b> se elimino con éxito</td>
									</tr>									
                                </table>                                
							</td>
                        </tr>
