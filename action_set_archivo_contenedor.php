...
$base->execute_query("SELECT * FROM adjunto WHERE id_noticia = ".$_POST['id_noticia']." AND principal = 2;"); // principal = 2 significa archivo contenedor

$error_delete = false;

//vemos si hay algun archivo contenedor
...
if (!($tamano_archivo_nuevo < 50000000)) //  50 M
{
...
{
    $error_upload= false;
    if (move_uploaded_file($_FILES['archivo']['tmp_name'], $url."/ID".$idnoticia."_cont_".$nombre_archivo_nuevo))
    {
        $mensaje = "exito";
    }
    else
...
if($mensaje == "exito")
{
    ...
    $datos_archivo = array("id_adjunto"=>"",
                                       ...
                                       "nombre_archivo"=>"ID".$idnoticia."_cont_".$nombre_archivo_nuevo,
...
{
    header("Location:set_archivo_contenedor.php?id_noticia=".$idnoticia."&id_tipo_fuente=".$tipofuente."&mensaje=".$mensaje);
}
else
{
