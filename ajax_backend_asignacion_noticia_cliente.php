...
if (isset($_GET['action']))
{
    switch($_GET['action'])
    {
            ...
            else // si hay temas
            {
                ...
                foreach ($arreglo_temas as  $tema)
                {
                    ...
                    else // si hay noticias
                    {
                        ...
                        $new_back[] .= '<table width="950" border="0" align="center">
                                      <tr class="header2">
                                        ...
                                        <td><div align="center">Tendencia</div></td>
                                        <td width="75">&nbsp;</td>
                                        <td width="110">&nbsp;</td>
                                      </tr>';
                        ...
                        foreach ($arreglo_noticias as  $noticia)
                        {
                            ...
                            $new_back[] .= '<td class="row1"><div align="center"><a href="ver_noticia_selector_basic.php?id_tipo_fuente='.$noticia->getId_tipo_fuente().'&id_noticia='.$noticia->getId().'" target="_blank" class="label5">'.$noticia->getId().'</a><br />
                    ...
                    Seccion: '.$noticia->getSeccion().'</div></td>';
                            ...
                            $new_back[] .= '<td class="row1">
                                          <div id="div_boton_'.$noticia->getId().'" align="center">
                                            <input type="submit" name="actualizar" id="actualizar_'.$noticia->getId().'" value="Modificar" onclick="modificaNoticia('.$noticia->getId().','.$_POST['id_cliente'].','.$noticia->getId_tema().','.$noticia->getId_tendencia().')" />
                                          </div></td>';
                            $new_back[] .= '<td class="row1">
                                          <div align="center">
                                            <input type="submit" name="quitar" id="quitar_'.$noticia->getId().'" value="Quitar de Portal" onclick="quitaNoticia('.$noticia->getId().','.$_POST['id_cliente'].','.$noticia->getId_tema().','.$noticia->getId_tendencia().')" />
                                          </div></td>';
                            $new_back[] .= '</tr>';
        ...
        case "get_temas":
            ...
            //generamos la salida
            $new_back = array();
            $new_back[] .='<select name="id_tema_'.$_POST['id_noticia'].'" style="width:155px; font-size:x-small;" id="id_tema_'.$_POST['id_noticia'].'">';
            foreach($arreglo_temas as $tema)
            {
        ...
        case "update":
            $output = "";
            $base->execute_query("UPDATE asigna SET id_tema =".$_POST['id_tema'].", id_tendencia=".$_POST['id_tendencia']." WHERE id_empresa=".$_POST['id_empresa']." AND id_noticia = ".$_POST['id_noticia']);
            $output = "exito";
         ...
         case "quit":
            $output = "";
            $base->execute_query("DELETE FROM asigna WHERE id_empresa=".$_POST['id_empresa']." AND id_noticia = ".$_POST['id_noticia']." LIMIT 1");
            $output = "exito";
