...
class ColumnaPolitica
{
    ...
    {
        $this->id = $datos['id_columna_politica'];
    ...
    function SQL_insert()
    {
        $query_insert = sprintf("INSERT INTO columna_politica (fecha, titulo, autor, contenido, imagen_jpg, archivo_pdf, id_fuente)
    ...
    function SQL_update()
    {
        $query_update =sprintf("UPDATE columna_politica SET
                                                          fecha=%s,
                                                          titulo=%s,
                                                          autor=%s,
                                                          contenido=%s,
                                                          ...
                                                          archivo_pdf=%s,
                                                          id_fuente=%s
                             WHERE id_columna_politica=%s LIMIT 1",
            $this->GetSQLValueString($this->fecha, "date"),
            $this->GetSQLValueString($this->titulo, "text"),
            $this->GetSQLValueString($this->autor, "text"),
            $this->GetSQLValueString($this->contenido, "text"),
    ...
    function SQL_delete()
    {
        $query_delete =sprintf("DELETE FROM columna_politica WHERE id_columna_politica =%s LIMIT 1",
            $this->GetSQLValueString($this->id, "int"));
        return $query_delete;
    }
}
