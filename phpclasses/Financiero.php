...
class Financiero
{
    ...
    {
        $this->id = $datos['id_portada_financiera'];
    ...
    function SQL_insert()
    {
        $query_insert = sprintf("INSERT INTO portada_financiera (fecha, imagen,id_fuente)
                                 VALUES (%s, %s, %s)",
    ...
    function SQL_update()
    {
        $query_update =sprintf("UPDATE portada_financiera SET fecha=%s, imagen=%s, id_fuente=%s
    ...
    function SQL_delete()
    {
        $query_delete =sprintf("DELETE FROM portada_financiera WHERE id_primera_plana =%s LIMIT 1",
            $this->GetSQLValueString($this->id, "int"));
        return $query_delete;
    }
}
