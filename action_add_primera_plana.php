...
$url = "data/primera_plana"; // donde se copian los documentos en el servidor
...
{
    $mensaje ="Error: El archivo excede el tama̱o limite";
    $error_upload = true;
}
else
{
...
if($mensaje == "exito")
{
    ...
    $pPlana = new PrimeraPlana($datos);
    $base->execute_query($pPlana->SQL_insert());
}

$base->close();
header("Location: agrega_primeraplana.php?&error_upload=".$error_upload."&mensaje=".$mensaje);
}
/****************************************************************************
 *
...
echo $query = "SELECT * FROM primera_plana WHERE id_primera_plana =".$row_id." LIMIT 1;";

$base->execute_query($query);
$pPlana = new PrimeraPlana($base->get_row_assoc());
...
$base->close();
header("Location: edit_primeraplana.php?id_pp=".$row_id."&error_upload=".$error_upload."&mensaje=".$mensaje);
}
else
{
   $base->close();
